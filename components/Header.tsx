...
const NavItem: React.FC<{ to: string; children: React.ReactNode; onClick?: () => void }> = ({ to, children, onClick }) => {
    ...
    return (
        <NavLink
            to={to}
            onClick={onClick}
            className={`relative px-3 py-2 text-sm font-medium transition-colors duration-200 block md:inline-block ${
                isActive 
            ...
            <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center">
                    ...
                    <NavLink className="flex items-center gap-2 group z-50 relative" to="/" onClick={() => setIsMobileMenuOpen(false)}>
                        <div className="bg-white text-primary-dark p-1 rounded-lg shadow-md group-hover:scale-105 transition-transform">
                ...
                <div 
                    className={`md:hidden absolute left-0 right-0 top-full bg-primary-dark dark:bg-background-dark border-t border-white/10 shadow-xl overflow-hidden transition-all duration-300 ease-in-out ${
